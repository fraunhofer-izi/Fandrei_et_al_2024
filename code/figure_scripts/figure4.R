...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Fig. 4B: CD4 CAR / CD8 CAR ratio boxplots
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Fig. 4C : non-transduced CD4 / CD8  ratio longitudinal
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
total_cd4_cd8_ratio <- ratio_total_cd3 %>%
...
# Compose Fig. 4 C + D
pl_ratio <- ratio_bridging_binary + total_cd4_cd8_ratio +
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Fig. 4D : Cytotox per bridging
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

# Comparison all CAR-T vs non-transduced T
cytotox_df %>%
  filter(name!="percentage_cart") %>%
  group_by(name) %>%
  summarize(median = median(value, na.rm=T), min(value, na.rm=T), max(value, na.rm=T))

kruskal.test(cytotox_df[which(cytotox_df$name!="percentage_cart"),]$value~cytotox_df[which(cytotox_df$name!="percentage_cart"),]$name)

cytotox_plot <- cytotox_df %>% 
  filter(!name %in% c("percentage_cart", "U266 control")) %>%
  mutate(name = factor(car::recode(name, "'U266 + nRI'='T cell culture (d7)';'U266 + vRI'='T cell culture (d0)' "), levels = c('T cell culture (d0)', 'T cell culture (d7)'))) %>%
  ...
  ylab("% U266 killing rate (24h incubation)") +
  guides(fill=guide_legend(nrow=1)) +
  stat_compare_means(label = "p.format", vjust=-4) +
  geom_pwc(label="p.signif", hide.ns="p", vjust=.5)

cytotox_df %>%
  filter(name=="U266 + vRI", therapy_before_cart == "SLAMF7") %>%
  group_by(name) %>%
  summarize(median = median(value, na.rm=T), min(value, na.rm=T), max(value, na.rm=T))

ggsave2(
  "figures/main/figure_4/cytotox_bridging.png",
  height=80, width=93, units="mm", dpi=400, bg="white", scale=1.7
)
...
sbcma_pl = elisa_df[,c("patient_id", "Day", "BCMA [ng/mL]")] %>%
  left_join(clin_data[,c("patient_id", "treatment_before_apheresis", "RESPONSE_CONSENSUS_1")]) %>%
  filter(!Day %in% c("Day 7" , "Day 14")) %>%
  mutate(group = factor(case_when(
    treatment_before_apheresis == "Teclistamab" ~ "teclistamab",
...
ggsave2(
  ...
  sbcma_pl,
  width = 105, height = 84, dpi = 400, bg = "white", units = "mm", scale = 1.6
)
